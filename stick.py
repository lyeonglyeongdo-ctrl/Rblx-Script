...
async def unsticky(interaction: discord.Interaction):
    ...
    if guild_id not in sticky_data or \
       channel_id not in sticky_data[guild_id]:
    ...
    sticky_data[guild_id].pop(channel_id, None)
    sticky_data.pop(channel_id, None)
...
async def delayed_sticky(guild_id, channel, version):
    ...
    if sticky_data.get(channel.id) != version:
        return
    ...
    try:
        old = await channel.fetch_message(
            sticky_data[guild_id][channel.id]
        )
        await old.delete()
    except:
        pass
...
async def on_message(message):
    ...
    if guild_id in sticky_data and \
       channel_id in sticky_data[guild_id]:
        ...
        current_version = sticky_data.get(channel_id, 0) + 1
        sticky_data[channel_id] = current_version
